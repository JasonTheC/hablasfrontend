...
def sanitize_title_for_key(title):
    ...
    if not title: title = "untitled_chapter"
    key = str(title).lower()
...
def extract_sentences_from_epub(epub_path):
    ...
    chapters_content = {}
    preface_assigned = False # Tracks if the primary "preface" key has been used
    used_keys = set() # To track all keys for de-duplication
    
    # Sort sections by order, just in case
    ...
    for section_data in sections_to_process:
        ...
        # 1. Determine if this section should be THE "preface"
        # It's THE preface if no preface has been assigned yet, AND
        # (it's explicitly a preface OR it's the very first section overall)
        is_this_the_preface = not preface_assigned and \
                              (is_explicit_preface_flag or not chapters_content) # not chapters_content ensures first item can be preface

        if is_this_the_preface:
            current_key_base = "preface"
            preface_assigned = True
        else:
            # 2. For other sections, use sanitized title
            current_key_base = sanitize_title_for_key(title)
            # If sanitize_title_for_key returns "untitled_chapter", that will be the base.
            # If the sanitized title happens to be "preface", but this section is not THE preface,
            # the collision handling below will add a suffix (e.g., "preface_1").
        
        # 3. Ensure key uniqueness
        final_key = current_key_base
        suffix = 1
        while final_key in used_keys:
            final_key = f"{current_key_base}_{suffix}"
            suffix += 1
